...
import { assetJson } from "./demoJsonTesting1";
...
});AFRAME.registerComponent('scene-loaded-check', {
  init: function () {
    // Reference to the scene element
    const sceneEl = this.el;

    // Event listener for the 'loaded' event on the scene
    const onSceneLoaded = () => {
      console.log('A-Frame scene is loaded!');
      sceneLoaded=true;
      // You can perform actions here after the scene is loaded.
    };

    // Add the 'loaded' event listener to the scene element
    sceneEl.addEventListener('loaded', onSceneLoaded);

    // Cleanup function to remove the event listener when the component is detached
    this.el.sceneEl.addEventListener('detached', () => {
...
console.log(timeout)
  ...
  }, [currentDivision, currentPhoneAssetIndex, lastSkipTime, video, isPlaying,timeout]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    assetsVisible,
    lastSkipTime,
    playNextAssets, timeout //updated dependecy might experinece some functionality changes because of it
  ...
  useEffect(() => {
    ...
    if (isPlaying && currentDivision!=="apploading" ) {
      buttonEntity.setAttribute("visible", "true");
    }
  ...
  useEffect(()=>{
    if(currentDivision==="appintro"){
        if(currentPhoneAssetIndex===1 || currentPhoneAssetIndex===2 || currentPhoneAssetIndex===1){
           setDisplayBg(false)
        }
        else{
            setDisplayBg(true);
  ...
  return (
    <div className="scene-container" onClick={()=>{setMute(false)}}>
      ...
      <div className="demo-subs-container appdemo-subs-container">
        <div className="previous-skip-container">
          <button
            className="next-btn"
            ref={skipBtnRef}
            style={{

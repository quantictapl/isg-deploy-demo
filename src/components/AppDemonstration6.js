...
function AppDemonstration() {
  ...
  useEffect(() => {
    ...
    button.play();
    if(currentDivision==="recpay" || phoneScreenVideo.muted===true || phoneScreenVideo.src.includes("mp4")){
  ...
  useEffect(() => {
    ...
    video.addEventListener("ended", handleAudioEnded);
    return () => {
  ...
  useEffect(()=>{
    ...
    if(currentDivision==="nullAssets" && currentPhoneAssetIndex===1){
      ...
      if((currentDivision==="appintro" && currentPhoneAssetIndex===1 )|| (currentDivision==="appintro" && currentPhoneAssetIndex===2 )|| (currentDivision==="appintro" && currentPhoneAssetIndex===3 )){;
        esy.removeAttribute("animation-mixer");
  ...
  },[audio, currentDivision,  currentPhoneAssetIndex, lastSkipTime, video,isPlaying])
 
  // useEffect(() => {
  //   // Access the video element using the ref
  //   const video = subsVideoRef.current;

  //   // Add an event listener to the video element for when it's loaded and can play.
  //   const onCanPlay = () => {
  //     // Once the video can play, remove the placeholder source.
  //     video.removeEventListener('canplay', onCanPlay);
  //     video.remove();
  ...
  //   console.log("current subs video src=",currentPhoneAsset.subsVideo.videoElement,)
  //   video.src = currentPhoneAsset.subsVideo.videoElement;
  ...
  //     video.removeEventListener('canplay', onCanPlay);
  //   };
  // }, [currentPhoneAsset.subsVideo.videoElement]); // Empty dependency array means this effect runs once, similar to componentDidMount
...
//   })
  ...
  <div class="smart-dropdown">
    <div id="dropdown-trigger" ref={dropDownTriggerRef} class="smart-dropdown" onClick={handleDropdownClick}><span className="smart-dropdown-title">Select Demo</span></div>
        ...
        <a-entity
          ...
          >
            ...
            //   ref={animationButtonRef}
              material="shader: flat; color:white; side: double; transparent: true; opacity: 1; src:#subsVideo;"
              geometry="primitive: plane; width: 5.45; height: 2.96"
              position={`0 -0.28 -0.86971`}
              rotation="0 -180 0"
              scale="-0.2 0.2 0.2"

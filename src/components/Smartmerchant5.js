...
// const merchantTv="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/videos/MerchantTv1.mp4";
...
function SmartMerchant({ esy,videos,smartmerchantBg}) {
  console.log("smartmerchant bg",smartmerchantBg)
  ...
  useEffect(() => {
    ...
    function timeupdate() {
      ...
      if(inInterval && video.src===videoSources[1]){//video.src===videoSources[1]
  ...
  }, [tvVideoVisible, isPlaying, isPaused, videoEnded, navigate, cameraRotationEnded]);//new dependecy included isTVVideoEnded Oct 05 2023
  ...
  const handlePreviousSubsClick = () => {
    ...
    if (previousSubsVideoIndex>=1) {
      ...
      console.log(previousSubsVideoIndex)
     // Call the callback function with the next video index
    } else{
...
useEffect(()=>{
  ...
  }, [currentSubsVideoIndex, cameraRotationEnded]);
  useEffect(() => {
    if (videoEnded) {
      setShowPlayButton(false);
  ...
  useEffect(() => {
    ...
    const checkModelLoaded=()=>{
      placeholderEntity.setAttribute("visible", false);
      actualModelEntity.setAttribute("visible", true);
      actualModelEntity.setAttribute('animation-mixer', 'clip:; loop: repeat; repetitions: Infinity;')
    ...
    <div className="scene-container">
      ...
      {/* <SubsLogic  isPlaying={isPlaying} tvVideoVisible={tvVideoVisible} onPlayPauseClick={handlePlayPauseClick}
        ...
        isTVVideoEnded={isTVVideoEnded}
         /> */}
         <div class="smart-dropdown" ref={dropDownTriggerRef} onClick={handleDropdownClick}>
          <div
            id="dropdown-trigger"
  ...
  <a-entity id="tv-border" material="shader: flat;  alphaTest:0.5;  src:#tv-border; transparent: true;" geometry="primitive: plane; width: 4; height: 2.05" position="-0.036 0.100 -13.814" rotation="3.500 -1.362 0.000" scale="7.5 7.5 7.5"></a-entity>
    ...
    <a-animation attribute="rotation" to="0 360 0" dur="1000" easing="linear" repeat="indefinite"></a-animation>
      ...
      {/* <a-entity
        ...
        <button className="logout-btn-smart-intro" onClick={logoutHandler}>
          Logout
        </button>
      </Scene>
    </div>

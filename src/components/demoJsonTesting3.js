import React from 'react'

// import appimg1 from "../SmartMerchantAssets/apploading/appimages/appLoadingImg1.png";
// import appimg2 from "../SmartMerchantAssets/apploading/appimages/appLoadingImg2.png";
// import appimg3 from "../SmartMerchantAssets/apploading/appimages/appLoadingImg3.png";

import buttonvideo from "../videos/circle1.mp4";
// import qrAudio from "../SmartMerchantAssets/Audios/qrAudio.mp3";
// import appLoading2Audio from "../SmartMerchantAssets/Audios/apploading2Audio.mp3";
// import appLoading3Audio from "../SmartMerchantAssets/Audios/apploading3Audio.mp3";
// import appIntro1Audio from "../SmartMerchantAssets/Audios/AppIntro1Audio.mp3";
// import appIntro2Audio from "../SmartMerchantAssets/Audios/AppIntro2Audio.mp3";


// import userreg1Audio from "../SmartMerchantAssets/Audios/userreg1Audio.mp3";
// import userreg2Audio from "../SmartMerchantAssets/Audios/userreg2Audio.mp3";
// import userreg3Audio from "../SmartMerchantAssets/Audios/userreg3Audio.mp3";


// import onboarding1Audio from "../SmartMerchantAssets/Audios/onboarding1Audio.mp3";
// import onboarding2Audio from "../SmartMerchantAssets/Audios/onboarding2Audio.mp3";


// import paymentacceptance1Audio from "../SmartMerchantAssets/Audios/paymentAcceptance1Audio.mp3";
// import paymentacceptance2Audio from "../SmartMerchantAssets/Audios/paymentAcceptance2Audio.mp3";

// import paymentmethod1Audio from "../SmartMerchantAssets/Audios/payment1Audio.mp3";
// import paymentmethod2Audio from "../SmartMerchantAssets/Audios/payment2Audio.mp3";
// import paymentmethod3Audio from "../SmartMerchantAssets/Audios/payment3Audio.mp3";
// import paymentmethod4Audio from "../SmartMerchantAssets/Audios/payment4Audio.mp3";
// import paymentmethod5Audio from "../SmartMerchantAssets/Audios/payment5Audio.mp3";
// import paymentmethod6Audio from "../SmartMerchantAssets/Audios/payment6Audio.mp3";
// import paymentmethod7Audio from "../SmartMerchantAssets/Audios/payment7Audio.mp3";

// import rtp1Audio from "../SmartMerchantAssets/Audios/rtp1Audio.mp3";
// import rtp2Audio from "../SmartMerchantAssets/Audios/rtp2Audio.mp3";
// import rtp3Audio from "../SmartMerchantAssets/Audios/rtp3Audio.mp3";
// import rtp4Audio from "../SmartMerchantAssets/Audios/rtp4Audio.mp3";

// import recpay1Audio from "../SmartMerchantAssets/Audios/recpay1Audio.mp3";
// import recpay2Audio from "../SmartMerchantAssets/Audios/recpay2Audio.mp3";
// import recpay3Audio from "../SmartMerchantAssets/Audios/recpay3Audio.mp3";

// import refund1Audio from "../SmartMerchantAssets/Audios/refund1Audio.mp3";
// import refund2Audio from "../SmartMerchantAssets/Audios/refund2Audio.mp3";
// import refund3Audio from "../SmartMerchantAssets/Audios/refund3Audio.mp3";

// import qrcode from "../SmartMerchantAssets/apploading/appimages/Qrcode.PNG";
// import appintro1 from "../SmartMerchantAssets/appintro/appintroimages/appIntroImg1.png";
// import appintro2 from "../SmartMerchantAssets/appintro/appintroimages/appIntroImg2.png";
// import appintro3 from "../SmartMerchantAssets/appintro/appintroimages/appIntroImg3.png";
// import appintro4 from "../SmartMerchantAssets/appintro/appintroimages/appIntroImg4.png";
// import appintro5 from "../SmartMerchantAssets/appintro/appintroimages/appIntroImg5.png";

// import userregimg1 from "../SmartMerchantAssets/userreg/UserRegImages/userregImg1.png";
// import userregimg2 from "../SmartMerchantAssets/userreg/UserRegImages/userregImg2.png";
// import userregimg3 from "../SmartMerchantAssets/userreg/UserRegImages/userregImg3.png";

// import paymentmethodimg1 from "../SmartMerchantAssets/paymentmethod/paymentmethodimg/paymentMethodImg1.png";
// import paymentmethodimg2 from "../SmartMerchantAssets/paymentmethod/paymentmethodimg/paymentMethodImg2.png";
// import paymentmethodimg3 from "../SmartMerchantAssets/paymentmethod/paymentmethodimg/paymentMethodImg3.png";
// import paymentmethodimg4 from "../SmartMerchantAssets/paymentmethod/paymentmethodimg/paymentMethodImg4.png";
// import paymentmethodimg5 from "../SmartMerchantAssets/paymentmethod/paymentmethodimg/paymentMethodImg5.png";
// import paymentmethodimg6 from "../SmartMerchantAssets/paymentmethod/paymentmethodimg/paymentMethodImg6.png";
// import paymentmethodimg7 from "../SmartMerchantAssets/paymentmethod/paymentmethodimg/paymentMethodImg7.png";

// import rtpimg1 from "../SmartMerchantAssets/rtp/rtpimages/rtpImg1.png";
// import rtpimg2 from "../SmartMerchantAssets/rtp/rtpimages/rtpImg2.png";
// import rtpimg3 from "../SmartMerchantAssets/rtp/rtpimages/rtpImg3.png";
// import rtpimg4 from "../SmartMerchantAssets/rtp/rtpimages/rtpImg4.png";

// import refundimg1 from "../SmartMerchantAssets/refundprocess/refundImg1.png";
// import refundimg2 from "../SmartMerchantAssets/refundprocess/refundImg2.png";
// import refundimg3 from "../SmartMerchantAssets/refundprocess/refundImg3.png";

import invisible from "../SmartMerchantAssets/invisible.glb";
// import appHandSingle from  "../SmartMerchantAssets/appintro/app_hand_single.glb";
// import appHandPanSingle from "../SmartMerchantAssets/userreg/app_hand_pan_single.glb";
// import appHandCardSingle from "../SmartMerchantAssets/paymentmethod/app_hand_card_single.glb";
// import recpayimg1 from "../SmartMerchantAssets/recordpayment/recpayImg1.png";

// import recpayVidIn1 from "../SmartMerchantAssets/recordpayment/recpayIn1.mp4";
// import recpayVidIn2 from "../SmartMerchantAssets/recordpayment/recpayIn2.mp4";
// import recpayVidIn3 from "../SmartMerchantAssets/recordpayment/recpayIn3.mp4";

// import blankAudio from "../SmartMerchantAssets/videos/Dialogs/blankAudio.mp3";
export const esy="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Esy.glb";
export const esyHand="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/isy_hand.glb";
const appHandSingle="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/handassets/app_hand_single.glb"
const appHandPanSingle="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/handassets/app_hand_pan_single.glb"
const appHandCardSingle="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/handassets/app_hand_card_single.glb"

const qrAudio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/qrAudio.mp3"
const appLoading2Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/apploading2Audio.mp3";
const appLoading3Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/apploading3Audio.mp3";
const appIntro1Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/AppIntro1Audio.mp3";
const appIntro2Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/AppIntro2Audio.mp3"

const userreg1Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/userreg1Audio.mp3";
const userreg2Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/userreg2Audio.mp3";
const userreg3Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/userreg3Audio.mp3"

const  onboarding1Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/onboarding1Audio.mp3"
const  onboarding2Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/onboarding2Audio.mp3"

const paymentacceptance1Audio ="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/paymentAcceptance1Audio.mp3";
const paymentacceptance2Audio ="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/paymentAcceptance2Audio.mp3";

const paymentmethod1Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/payment1Audio.mp3"
const paymentmethod2Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/payment2Audio.mp3"
const paymentmethod3Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/payment3Audio.mp3"
const paymentmethod4Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/payment4Audio.mp3"
const paymentmethod5Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/payment5Audio.mp3"
const paymentmethod6Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/payment6Audio.mp3"
const paymentmethod7Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/payment7Audio.mp3"

const rtp1Audio = "https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/rtp1Audio.mp3";
const rtp2Audio = "https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/rtp2Audio.mp3";
const rtp3Audio = "https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/rtp3Audio.mp3";
const rtp4Audio = "https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/rtp4Audio.mp3";

const recpay1Audio= "https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/recpay1Audio.mp3";
const recpay2Audio= "https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/recpay2Audio.mp3";
const recpay3Audio= "https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/recpay3Audio.mp3";

const refund1Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/refund1Audio.mp3";
const refund2Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/refund2Audio.mp3";
const refund3Audio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/refund3Audio.mp3";
const qrcode="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/appimages/Qrcode.PNG";
const appimg1= "https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/appimages/appLoadingImg1.png";
const appimg2= "https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/appimages/appLoadingImg2.png";
const appimg3= "https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/appimages/appLoadingImg3.png";

const appintro1="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/appintroimages/appIntroImg1.png";
const appintro2="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/appintroimages/appIntroImg2.png";
const appintro3="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/appintroimages/appIntroImg3.png";
const appintro4="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/appintroimages/appIntroImg4.png";
const appintro5="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/appintroimages/appIntroImg5.png";


const userregimg1 ="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/UserRegImages/userregImg1.png";
const userregimg2 ="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/UserRegImages/userregImg2.png";
const userregimg3 ="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/UserRegImages/userregImg3.png";

const paymentmethodimg1="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/paymentmethodimg/paymentMethodImg1.png";
const paymentmethodimg2="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/paymentmethodimg/paymentMethodImg2.png";
const paymentmethodimg3="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/paymentmethodimg/paymentMethodImg3.png";
const paymentmethodimg4="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/paymentmethodimg/paymentMethodImg4.png";
const paymentmethodimg5="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/paymentmethodimg/paymentMethodImg5.png";
const paymentmethodimg6="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/paymentmethodimg/paymentMethodImg6.png";
const paymentmethodimg7="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/paymentmethodimg/paymentMethodImg7.png";

const rtpimg1="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/rtpimages/rtpImg1.png";
const rtpimg2="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/rtpimages/rtpImg2.png";
const rtpimg3="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/rtpimages/rtpImg3.png";
const rtpimg4="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/rtpimages/rtpImg4.png";

const refundimg1="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/refundprocess/refundImg1.png";
const refundimg2="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/refundprocess/refundImg2.png";
const refundimg3="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/refundprocess/refundImg3.png";

const recpayimg1="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/recordpayment/recpayImg1.png";

const recpayVidIn1="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/recordpayment/recpayIn1.mp4";
const recpayVidIn2="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/recordpayment/recpayIn2.mp4";
const recpayVidIn3="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/recordpayment/recpayIn3.mp4";

const blankAudio="https://isg-assets.s3.ap-south-1.amazonaws.com/SmartMerchantAssets/Audios/blankAudio.mp3";

export  const assetJson = {
    apploading: {
      phoneImages: [
        {
          name: "appimg1",
          imagePath: "",
          buttonPosition: { x: 0.1, y: 0.088 }, //y-0.278 x0.691
          width: 3.8,
          height: 3.8,
          scale:{x: 0, y: 0, z: 0},
          position: { x: 0.00396, y: -0.01997, z: -0.96379 },
          rotation: {
            x: 2.8447354528245374,
            y: -0.4526366581533504,
            z: -0.1753250853100319,
          },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "appimg2",
          imagePath: appimg2,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "appimg3",
          imagePath: appimg3,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        // {
        //   name: "appimg4",
        //   imagePath: appimg4,
        //   buttonPosition: { x: 0.664, y: -0.261 },
        //   width:2,
        //   height:3.8,
        //   position:{x:0.65229, y:-0.006, z:-0.98319},
        //   rotation:{x:0.529, y:-8.097, z:0.751}
        // },
      ],
      phoneAssets: [
        {
          name: "apploading1",
          imagePath: invisible,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.97307, y: -3.08774, z: -3.43762 },
          rotation: {
            x: -61.526499872328316,
            y: -169.76051920372137,
            z: -0.8416750010471793,
          },
          phoneScreen:{
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: qrAudio,
            subtitles:[
                { "start": 0.00, "end": 3, "text": "Our ISGPay Smartmerchant App is a powerful tool" },
                { "start": 4, "end": 7, "text": "that empowers businesses to manage their payments with ease." },
                { "start": 7, "end": 13, "text": "Use your smartphone's camera to scan the QR code above to download the app" },
                { "start": 14, "end": 17, "text": "It's your gateway to a world of  convinence" },
              ],
          },
        },
        {
          name: "apploading2",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: appLoading2Audio,
            subtitles:[
                { "start": 0.00, "end": 3, "text": "Step into the future of effortless transactions with our cutting edge" },
                { "start": 4, "end": 6, "text": "ISGPay Smartmerchant Payments App." },
              ],
          },
        },
        {
          name: "apploading3",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: appLoading3Audio,
            subtitles:[
                { "start": 0.00, "end": 3, "text": "Welcome to a new era of convinence right at your finger tips" },
                { "start": 4, "end": 10, "text": "Experience hassle free payments and explore a world of possibilities today!" },
              ],
          },
        },
        // {
        //   name: "apploading4",
        //   imagePath: appLoading4,
        //   animation: "",
        //   scale: { x: 0.034, y: 0.034, z: 0.034 },
        //   position: { x: -0.97307, y: -3.08774, z: -3.43762 },
        //   rotation: {
        //     x: -61.526499872328316,
        //     y: -169.76051920372137,
        //     z: -0.8416750010471793,
        //   },
        //   subsVideo: {
        //    parent2,
        //     audio: demoAudio,
        //   },
        // },
      ],
    },
    nullAssets: {
      phoneImages: [
        {
          name: "nullImg1",
          imagePath: "",
          buttonPosition: { x: 0.023, y: 0.088 },
          width: 0,
          height: 0,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "nullImg2",
          imagePath: "",
          buttonPosition: { x: -0.566, y: 0.295 },
          width: 0,
          height: 0,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        // {
        //   name: "nullImg3",
        //   imagePath: "",
        //   buttonPosition: { x: -0.75, y: 0.350 },
        //   width: 0,
        //   height: 0,
        //   position: { x: 0.65229, y: -0.006, z: -0.98319 },
        //   rotation: { x: 0.529, y: -8.097, z: 0.751 },
        // },
      ],
      phoneAssets: [
        {
          name: "nullAsset1",
          imagePath: "",
          animation: "Animation",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -1.1, y: 0.388, z: -0.877 },
          rotation: {
            x: -62.723,
            y: -171.366,
            z: 2.282,
          },
          phoneScreen:{
            scale:{x:0, y:0, z:0},
            position:{x:-0.10769, y:-0.0083, z:-0.7636},
            rotation:{x:-2.3073010409918253, y:4.6564280010282, z:-1.7647100090029355},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: onboarding1Audio,
            subtitles:[
                { "start": 0.00, "end": 2, "text": "Our app boasts an intuitive user interface" },
                { "start": 3, "end": 6, "text": "making it easy for you to navigate and access all of its functionalities." },
                { "start": 6, "end": 10, "text": "From setting up your account to processing transactions," },
                { "start": 11, "end": 15, "text": "we've made sure the app is a breeze to use." },
              ],
          },
        },
        {
          name: "nullAsset2",
          imagePath: "",
          animation: "Animation",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -1.1, y: 0.388, z: -0.877 },
          rotation: {
            x: -62.723,
            y: -171.366,
            z: 2.282,
          },
          phoneScreen:{
            scale:{x:0., y:0, z:0},
            position:{x:-0.10769, y:-0.0083, z:-0.7636},
            rotation:{x:-2.3073010409918253, y:4.6564280010282, z:-1.7647100090029355},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: onboarding2Audio,
            subtitles:[
                { "start": 0.00, "end": 4, "text": "Please select 'On Boarding' to know more!" },
              ],
          },
        },
        // {
        //   name: "nullAsset3",
        //   imagePath: "",
        //   animation: "Animation",
        //   scale: { x: 0.034, y: 0.034, z: 0.034 },
        //   position: { x: -1.1, y: 0.388, z: -0.877 },
        //   rotation: {
        //     x: -62.723,
        //     y: -171.366,
        //     z: 2.282,
        //   },
        //   subsVideo: {
        //    g2Dialog,
        //     audio: onboarding2Audio,
        //   },
        // },
      ],
    },
    appintro: {
      phoneImages: [
        {
          name: "appintro1",
          imagePath: appintro1,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "appintro2",
          imagePath: appintro2,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
          
        },
        {
          name: "appintro3",
          imagePath: appintro3,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "appintro4",
          imagePath: appintro4,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "appintro5",
          imagePath: appintro5,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
      ],
      phoneAssets: [
        {
          name: "appintroasset1",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position:{x:0, y:0, z:-10},
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: appIntro1Audio,
            subtitles:[
                { "start": 0.00, "end": 4, "text": "Embark on a personalised journey with us! Your preferred language matters" },
                { "start": 4, "end": 10, "text": "and we are thrilled to offer you a seamless experience in languages that resonate with you." },
                { "start": 10, "end": 16, "text": "Beyond English we extend our warm embrace with support for 8 regional languages." },
              ],
          },
        },
        {
          name: "appintroasset2",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: blankAudio,
            subtitles:[
                { "start": 0.00, "end": 3, "text": "" },
                { "start": 4, "end": 10, "text": "" },
              ],
          },
        },
        {
          name: "appintroasset3",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: blankAudio,
            subtitles:[
                { "start": 0.00, "end": 3, "text": "" },
                { "start": 4, "end": 10, "text": "" },
              ],
          },
        },
        {
          name: "appintroasset4",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: blankAudio,
            subtitles:[
                { "start": 0.00, "end": 3, "text": "" },
                { "start": 4, "end": 10, "text": "" },
              ],
          },
        },
        {
          name: "appintroasset5",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: appIntro2Audio,
            subtitles:[
                { "start": 0.00, "end": 5, "text": "ISGPay Smartmerchant's API architecture enables speedy," },
                { "start": 5, "end": 9, "text": "seamless and secure online onboarding at the time of downloading the app" },
                { "start": 9, "end": 11, "text": "using your registerd mobile number" },
              ],
          },
        },
      ],
    },
    userreg: {
      phoneImages: [
        {
          name: "userregimg1",
          imagePath: userregimg1,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "userregimg2",
          imagePath: userregimg2,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "userregimg3",
          imagePath: userregimg3,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
      ],
      phoneAssets: [
        {
          name: "userreg1",
          imagePath: appHandPanSingle,
          animation: "Animation",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position:{x:0, y:0, z:-10},
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: userreg1Audio,
            subtitles:[
                { "start": 0.00, "end": 3, "text": "There are different lists of documents to upload" },
                { "start": 3, "end": 8, "text": "depending on the type of company you select from the dropdown menu." },

              ],
          },
        },
        {
          name: "userreg2",
          imagePath: appHandPanSingle,
          animation: "Animation",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: userreg2Audio,
            subtitles:[
                { "start": 1, "end": 6, "text": "In India for instance, there is Instant Aaadhar, PAN and GST Verification. " },
                { "start": 6, "end": 9, "text": "You can put in your bank details and your IFSC  code" },
                { "start": 9, "end": 13, "text": "and your bank and bank branch will automatically reflect." },
              ],
          },
        },
        {
          name: "userreg3",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: userreg3Audio,
            subtitles:[
                { "start": 0.00, "end": 5, "text": "Take a picture of your store with your GPS enabled mobile and you will find over time," },
                { "start": 5.5, "end": 11, "text": "that this will allow you to geotag your mobile point of sales and terminal ID's." },
              ],
          },
        },
      ],
    },
    nullAssets1: {
      phoneImages: [
        {
          name: "nullImg1",
          imagePath: "",
          buttonPosition: { x: 0.023, y: 0.088 },
          width: 0,
          height: 0,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "nullImg2",
          imagePath: "",
          buttonPosition: { x: -0.706, y: 0.295 },
          width: 0,
          height: 0,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        // {
        //   name: "nullImg3",
        //   imagePath: "",
        //   buttonPosition: { x: -0.75, y: 0.350 },
        //   width: 0,
        //   height: 0,
        //   position: { x: 0.65229, y: -0.006, z: -0.98319 },
        //   rotation: { x: 0.529, y: -8.097, z: 0.751 },
        // },
      ],
      phoneAssets: [
        {
          name: "nullAsset1",
          imagePath: "",
          animation: "Animation",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -1.1, y: 0.388, z: -0.877 },
          rotation: {
            x: -62.723,
            y: -171.366,
            z: 2.282,
          },
          phoneScreen:{
            scale:{x:0, y:0, z:0},
            position:{x:-0.10769, y:-0.0083, z:-0.7636},
            rotation:{x:-2.3073010409918253, y:4.6564280010282, z:-1.7647100090029355},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position:{x:0, y:0, z:-10},
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: paymentacceptance1Audio,
            subtitles:[
                { "start": 0.00, "end": 4, "text": "ISGPay Smartmerchant is an all purpose payments acceptance application," },
                { "start": 4, "end": 10, "text": "otherwise known as omnichannel. Popular modes of payment like QR payments," },
                { "start": 10, "end": 15, "text": "Contactless or Tap and Pay using credit, debit and prepaid cards"},
                { "start": 15, "end": 22, "text": "and link payments or Request to pay sent via SMS or email are all enabled in our app." },
              ],
          },
        },
        {
          name: "nullAsset2",
          imagePath: "",
          animation: "Animation",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -1.1, y: 0.388, z: -0.877 },
          rotation: {
            x: -62.723,
            y: -171.366,
            z: 2.282,
          },
          phoneScreen:{
            scale:{x:0, y:0, z:0},
            position:{x:-0.10769, y:-0.0083, z:-0.7636},
            rotation:{x:-2.3073010409918253, y:4.6564280010282, z:-1.7647100090029355},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position:{x:0, y:0, z:-10},
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: paymentacceptance2Audio,
            subtitles:[
                { "start": 0.00, "end": 4, "text": "Please select 'Payment Acceptance' to know more!" },
              ],
          },
        },
        // {
        //   name: "nullAsset3",
        //   imagePath: "",
        //   animation: "Animation",
        //   scale: { x: 0.034, y: 0.034, z: 0.034 },
        //   position: { x: -1.1, y: 0.388, z: -0.877 },
        //   rotation: {
        //     x: -62.723,
        //     y: -171.366,
        //     z: 2.282,
        //   },
        //   subsVideo: {
        //    g2Dialog,
        //     audio: onboarding2Audio,
        //   },
        // },
      ],
    },
    paymentmethod: {
      phoneImages: [
        {
          name: "paymentimg1",
          imagePath: paymentmethodimg1,
          buttonPosition: { x: 0.108, y: -0.2 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "paymentimg2",
          imagePath: paymentmethodimg2,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "paymentimg3",
          imagePath: paymentmethodimg3,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "paymentimg4",
          imagePath: paymentmethodimg4,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "paymentimg5",
          imagePath: paymentmethodimg5,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "paymentimg6",
          imagePath: paymentmethodimg6,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "paymentimg7",
          imagePath: paymentmethodimg7,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
      ],
      phoneAssets: [
        {
          name: "paymentmethod1",
          imagePath: appHandCardSingle,
          animation: "",
          scale: { x: 0.60589, y: 0.60589, z: 0.60589 },
          position: { x: -0.549, y: -2.61681, z: -2.98667 },
          rotation: { x: -61.526499872328316, y: -169.76051920372137, z: -0.8416750010471793 },
          phoneScreen:{
            scale:{x:0.103, y:0.1150, z:0.117},
            position:{x:0.070, y:-0.0512, z:-0.742},
            rotation:{x:-3.125, y:3.442, z:0.318},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: paymentmethod1Audio,
            subtitles:[
                { "start": 0.00, "end": 7, "text": "Tap & Pay uses Near Field Communication(NFC) available on most of the android phones." },
                { "start": 7, "end": 13, "text": "Your customers can use Tap & Pay feature for purchasess up to a value of ₹5000." },

              ],
          },
        },
        {
          name: "paymentmethod2",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: paymentmethod2Audio,
            subtitles:[
                { "start": 0.00, "end": 4, "text": "BHIM UPI Option allows you to select the UPI QR." },
                { "start": 4, "end": 7, "text": "Using Quick Reach Codes or QRC's" },
                { "start": 7, "end": 11, "text": "the ISGPay SmartMerchant App makes a quick draw" },
                { "start": 11, "end": 13, "text": "on the customer's accounts and mobile wallets at checkout." },
              ],
          },
        },
        {
          name: "paymentmethod3",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: paymentmethod3Audio,
            subtitles:[
                { "start": 0.00, "end": 5, "text": "Once UPI QR is selected, a Static QRs will appear," },
                { "start": 5, "end": 10, "text": "which the customer can scan on their mobile phone and pay an amount inputted by them." },
              ],
          },
        },
        {
          name: "paymentmethod4",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: paymentmethod4Audio,
            subtitles:[
                { "start": 0.00, "end": 5, "text": "To generate a Dynamic QR, click on the option below," },
                { "start": 5, "end": 10, "text": "Enter the amount payable and the customer's mobile number if you would like to share the QR." },
              ],
          },
        },
        {
          name: "paymentmethod5",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: paymentmethod5Audio,
            subtitles:[
                { "start": 0.00, "end": 4, "text": "Customer can scan on their mobile phones and pay." },
              ],
          },
        },
        {
          name: "paymentmethod6",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: paymentmethod6Audio,
            subtitles:[
                { "start": 0.00, "end": 3, "text": "For the customers, want to pay upon delivery." },
                { "start": 3, "end": 7, "text": "you would click on 'Collect Request',fill in the customer VPA," },
                { "start": 7, "end": 10, "text": "the amount and when the payments need to be completed" },
              ],
          },
        },
        {
          name: "paymentmethod7",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: paymentmethod7Audio,
            subtitles:[
                { "start": 0.00, "end": 5, "text": "They simply accept or decline to pay on your push notification." },
              ],
          },
        },
      ],
    },
    rtpmethod: {
      phoneImages: [
        {
          name: "rtpimg1",
          imagePath: rtpimg1,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "rtpimg2",
          imagePath: rtpimg2,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "rtpimg3",
          imagePath: rtpimg3,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "rtpimg4",
          imagePath: rtpimg4,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
      ],
      phoneAssets: [
        {
          name: "rtpmethod1",
          imagePath: appHandSingle,
          animation: "Animation",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: rtp1Audio,
            subtitles:[
                { "start": 0.00, "end": 3, "text": "ISGPay SmartMerchant app facilitates Link Payments." },
                { "start": 4, "end": 9, "text": "You can send requests via SMS or email. Click 'SMS Pay'," },
                { "start": 9, "end": 14, "text": "Enter a valid customer mobile number and transaction amount. 'Send Request'" },
              ],
          },
        },
        {
          name: "rtpmethod2",
          imagePath: appHandSingle,
          animation: "Animation",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: rtp2Audio,
            subtitles:[
                { "start": 0.00, "end": 6, "text": "Pop-up notifications and SMS appears on their phone with the payment link." },

              ],
          },
        },
        {
          name: "rtpmethod3",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: rtp3Audio,
            subtitles:[
                { "start": 0.00, "end": 4, "text": "Pay Later Feature allows you to register customers" },
                { "start": 4, "end": 9, "text": "to whom you offer goods on credit, as per agreed payment cycles " },
              ],
          },
        },
        {
          name: "rtpmethod4",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: rtp4Audio,
            subtitles:[
                { "start": 0.00, "end": 5, "text": "Also it gives you visibility of all your debtors on one platform" },
                { "start": 5, "end": 10, "text": "so that you can record full or partial payments received" },
              ],
          },
        },
      ],
    },
    recpay: {
      phoneImages: [
        {
          name: "recpayvideo1",
          imagePath: recpayimg1,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:recpayVidIn1,
            scale:{x:0.14983, y:0.07264, z:0.01075},
            position: { x:0.64708, y:0.00007, z:-0.97569 },
            rotation: {x:0.657755548810185, y:-8.859073428312788, z:1.124143194046675},
          },
        },
        {
          name: "recpayvideo2",
          imagePath: recpayimg1,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:recpayVidIn2,
            scale:{x:0.14983, y:0.07264, z:0.01075},
            position: { x:0.64708, y:0.00007, z:-0.97569 },
            rotation: {x:0.657755548810185, y:-8.859073428312788, z:1.124143194046675},
          },
        },
        {
          name: "recpayvideo3",
          imagePath: recpayimg1,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:recpayVidIn3,
            scale:{x:0.14983, y:0.07264, z:0.01075},
            position: { x:0.64708, y:0.00007, z:-0.97569 },
            rotation: {x:0.657755548810185, y:-8.859073428312788, z:1.124143194046675},
          },
        },
      ],
      phoneAssets: [
        {
          name: "recpay",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:recpayVidIn1,
            scale:{x:0.114, y:0.053, z:0.117},
            position:{x:0.084, y:-0.017, z:-0.700},
            rotation:{x:-2.906, y:1.499, z:0.021},
          },
          subsVideo: {
            audio: recpay1Audio,
            subtitles:[
                { "start": 0.00, "end": 3, "text": "Our merchant dashboard displays the Name" },
                { "start": 3, "end": 8, "text": "and has an option to select single or multiple terminal ID's to view collections." },
                { "start": 8, "end": 14, "text": "It features the day's sale prominently with five most recent transactions." },

              ],
          },
        },
        {
          name: "recpay2",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:recpayVidIn2,
            scale:{x:0.114, y:0.053, z:0.117},
            position:{x:0.084, y:-0.017, z:-0.700},
            rotation:{x:-2.906, y:1.499, z:0.021},
          },
          subsVideo: {
            audio: recpay2Audio,
            subtitles:[
                { "start": 0.00, "end": 5, "text": "It also disaggregates every transaction made at your mobile points of sale" },
                { "start": 5, "end": 9, "text": "for whether they are successful,have failed, or whether they are pending." },
              ],
          },
        },
       
        {
          name: "recpay3",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:recpayVidIn3,
            scale:{x:0.114, y:0.053, z:0.117},
            position:{x:0.084, y:-0.017, z:-0.700},
            rotation:{x:-2.906, y:1.499, z:0.021},
          },
          subsVideo: {
            audio: recpay3Audio,
            subtitles:[
                { "start": 0.00, "end": 5, "text": "It also segments every transaction, based on Payments mode." },
              ],
          },
        },
      ],
    },
    // datahandling: {
    //   phoneImages: [
    //     {
    //       name: "datahandling",
    //       imagePath: datahandlingimg,
    //       buttonPosition: { x: 0.662, y: -0.285 },
    //       width: 2,
    //       height: 3.8,
    //       position: { x: 0.65229, y: -0.006, z: -0.98319 },
    //       rotation: { x: 0.529, y: -8.097, z: 0.751 },
    //     },
    //   ],
    //   phoneAssets: [
    //     {
    //       name: "datahandling",
    //       imagePath: datahandling,
    //       animation: "Animation",
    //       scale: { x: 0.034, y: 0.034, z: 0.034 },
    //       position: { x: -1.0364, y: -3.02252, z: -3.46246 },
    //       rotation: {
    //         x: -62.42890839965937,
    //         y: -172.72557579352335,
    //         z: 3.9614301955345113,
    //       },
    //       subsVideo: {
    //        parent2,
    //         audio: demoAudio,
    //       },
    //     },
    //   ],
    // },
    refundprocess: {
      phoneImages: [
        {
          name: "refundimg1",
          imagePath: refundimg1,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "refundim2",
          imagePath: refundimg2,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
        {
          name: "refundimg3",
          imagePath: refundimg3,
          buttonPosition: { x: 0.141, y: -0.214 },
          width: 2,
          height: 3.8,
          scale:{x: 0.178, y: 0.2, z: 0.2},
          position: { x: 0.65229, y: -0.006, z: -0.98319 },
          rotation: { x: 0.529, y: -8.097, z: 0.751 },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
      ],
      phoneAssets: [
        {
          name: "refundasset1",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: refund1Audio,
            subtitles:[
                { "start": 0.00, "end": 5, "text": "There's also a full or parital refund for successful transactions," },
              ],
          },
        },
        {
          name: "refundasset2",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position: { x: 0, y: 0, z: -10 },
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: refund2Audio,
            subtitles:[
                { "start": 0.00, "end": 5, "text": "which is OTP based if paid by UPI or Link Payment."},
              ],
          },
        },
        {
          name: "refundasset3",
          imagePath: appHandSingle,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.56215, y: -3.08923, z: -3.43251 },
          rotation: { x: -61.61244354159794, y: -173.50995501505744, z: 0.39476792084513723 },
          phoneScreen:{
            scale:{x:0.12336, y:0.1377, z:0.117},
            position:{x:0.085, y:-0.023, z:-0.712},
            rotation:{x:-2.999, y:0.012, z:-0.142},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position:{x:0, y:0, z:-10},
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: refund3Audio,
            subtitles:[
                { "start": 0.00, "end": 3, "text": "In the case of Tap and Pay" },
                { "start": 3, "end": 5, "text": "your customer would have to tap their card on the"},
                { "start": 5, "end": 9, "text": "ISGPay App-enabled mobile POS for a refund." },
              ],
          },
        },
      ],
    },
    null: {
      phoneImages: [
        {
          name: "null1",
          imagePath: "",
          buttonPosition: { x: 0, y: 0 },
          width: 3.8,
          height: 3.8,
          scale:{x: 0, y: 0, z: 0},
          position: { x: 0.00396, y: -0.01507, z: -0.96379 },
          rotation: {
            x: 2.8447354528245374,
            y: -0.4526366581533504,
            z: -0.1753250853100319,
          },
          phoneImg:{
            imagePath:"",
            scale:{x: 0, y: 0, z: 0},
            position: { x: 0, y: 0, z: -10 },
            rotation: { x: 0, y: 0, z: 0 },
          },
        },
      ],
      phoneAssets: [
        {
          name: "null1",
          imagePath: invisible,
          animation: "",
          scale: { x: 0.034, y: 0.034, z: 0.034 },
          position: { x: -0.97307, y: -3.08774, z: -3.43762 },
          rotation: {
            x: -61.526499872328316,
            y: -169.76051920372137,
            z: -0.8416750010471793,
          },
          phoneScreen:{
            scale:{x:0, y:0, z:0},
            position:{x:-0.231, y:-0.01277, z:-0.74997},
            rotation:{x:-2.756, y:3.434, z:0.238},
          },
          phoneScreenIn:{
            imagepath:invisible,
            scale:{x:0, y:0, z:0},
            position:{x:0, y:0, z:0},
            rotation:{x:0, y:0, z:0},
          },
          subsVideo: {
            audio: refund3Audio,
            subtitles:[
                { "start": 0.00, "end": 2, "text": "" },
              ],
          },
        },
      ],
    },
  };
  //y-0.278 x0.691